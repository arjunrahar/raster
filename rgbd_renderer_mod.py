...
parser.add_argument(
    ...
    eggbox, glue, holepuncher, iron, lamp, phone, car} (default ape)"
)
parser.add_argument(
...
class LineModRenderDB():
    ...
    def gen_pack_zbuf_render(self):
        ...
        for idx, RT in tqdm(enumerate(self.RT_lst)):
            ...
            #print("k",K.shape,"R", type(R), "T", T, "xyz", new_xyz.shape, "RT", RT.shape)
            new_xyz = np.dot(new_xyz, R.T) + T
            #print("new_xyz", R.T.shape)
            ...
            #np.savetxt('data.csv', (zbuf), delimiter=',')
            ...
            #print("msk",msk)
            #print("len np", len(np.where(msk.flatten() > 0)[0]))
            #print("np where",np.where(msk.flatten() > 0)[0])
            #print("msk flatten",np.where(msk.flatten() > 0))
            #print("msk true",(msk.flatten() > 0))
            #print("where", np.where(False))
            # for i in range(len(msk.flatten())):
            #     a = msk.flatten()
            #     if a[i]>0:
            #         print(a[i])
            #np.savetxt('data.csv', (msk.flatten()), delimiter=',')
